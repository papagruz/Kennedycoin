...
#if defined(NDEBUG)
# error "Kennedycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x76d8ef05155fc6225501ef38c9f97886b813cc37ae188407240666b96d1ccb6d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kennedycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kennedycoin Signed Message:\n";
...
{
    // Kennedycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kennedycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kennedycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8a69e5bfa197fadcc086e6a539d2790b357334326e3de67c2c24d291363b1051");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1532996432, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Kennedycoin is live!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04eb323ed1a223713ec2f4e667a56a930e1d55e982308026ae0df6d2aa21b1be5d0a0cc4a8680187e01862c8c7c9a298342fc960f2ed60b33e6e139935c6511500") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1533550950;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 9938224;

        if (fTestNet)
        {
            block.nTime    = 1533550893;
            block.nNonce   = 385661828;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xdd7354388b2e114bf3ed6e06d946854e9b9187bd08f6f473d671adcdaaa781ea"));
...
unsigned char pchMessageStart[4] = { 0x2b, 0x73, 0x27, 0x41 }; // Kennedycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KennedycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KennedycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KennedycoinMiner(CWallet *pwallet)
{
    printf("KennedycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Kennedycoin-miner");
    ...
    try { loop {
        ...
        printf("Running KennedycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KennedycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KennedycoinMiner, pwallet));
}
